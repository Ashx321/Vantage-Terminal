...
import { usePaperTrade } from '../lib/usePaperTrade'
import { formatRupees, formatChange, getChangeColor } from '../components/stockDisplay'
    ...
    <div className="bg-card border border-border rounded-xl p-4">
      ...
      <div className="flex gap-2 flex-wrap">
        <input
          ...
          className="flex-1 min-w-[140px] bg-card2 border border-border rounded-lg px-3 py-2 text-sm"
        />
        <input
          type="number"
          value={qty}
          ...
          placeholder="Qty"
          className="w-24 bg-card2 border border-border rounded-lg px-3 py-2 text-sm font-num"
        />
        <button
          onClick={handleBuy}
          disabled={busy}
...
export default function PaperTrade() {
  const { cash, positions, trades, loading, buy, sell } = usePaperTrade()
    ...
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <div className="text-[10.5px] text-muted">Started at &#8377;1,00,000</div>
        </div>
      </div>
      ...
      <div className="text-[10.5px] text-muted text-center mt-6">
        100% virtual — no brokerage connection, no real money, ever. Prices are real and live;
        the cash isn't.
      </div>
    </div>
  )
